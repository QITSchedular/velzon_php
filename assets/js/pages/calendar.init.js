...
$.ajax({
  ...
  data: { "flag": "load_events" },
  async: false,
  success: function(data){
    var value=data.split('*');
    for(var i=0;i<value.length-1;i++)
    {
      var abc=JSON.parse(value[i]);
      result={
          id: abc.id,
          url: abc.url,
          title: abc.title,
          start: new Date(abc.start),
          end:new Date(abc.end),
          allDay: abc.allDay,
          extendedProps: { calendar: abc.extendedProps },
          className:abc.extendedProps
        }
      events1.push(result);
    }
  }
